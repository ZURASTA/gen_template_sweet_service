defmodule <%= @project_name_camel_case %>.API do
    @moduledoc """
      The APIs for interfacing with the `<%= @project_name_camel_case %>.Service`.
    ...
    def defaults(opts) do
        Keyword.merge([
            server: Application.get_env(:<%= @project_name %>_api, :server, &(&1)),
            timeout: Application.get_env(:<%= @project_name %>_api, :timeout, 5000)
        ], opts)
    end
